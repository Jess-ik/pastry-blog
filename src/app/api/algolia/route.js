...
export async function POST(request) {
    ...
    try {
        ...
        const index = algoliaClient.initIndex("KitchenBook");

        // Get all articles from Prismic
        const articles = await prismicClient.getAllByType("recipe_page");

        // Map articles to Algolia records
        const articleRecords = articles.map((post) => ({
            objectID: post.id, // Unique identifier in algolia
            title: post.data.recipe_name, // Post title
            slug: post.uid, // Post URL slug
            image: post.data.recipe_hero_img, // Post featured image
            text: post.data.recipe_desc, // Post content transformed to search text
            tag: post.data.tag,
        }));
